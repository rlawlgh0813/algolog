...
- 이후 `#11`에서 JWT 검증 필터가 추가되면서 문제 등록 API도 인증 필요 정책을 따르게 되었습니다.
...
## #13 Visibility and ownership authorization

### 브랜치

- `feature/#13-visibility-authorization`

### 목적

공개/비공개 풀이 기록과 작성자 기반 수정/삭제 권한을 일관되게 처리합니다.

### 작업 내용

- 공개 풀이 기록 상세 조회를 비로그인 사용자에게 허용
- 공개 풀이 기록 상세 조회를 다른 로그인 사용자에게 허용
- 비공개 풀이 기록 상세 조회를 작성자에게만 허용
- 수정/삭제 작성자 전용 정책 유지
- 공개/비공개/작성자 케이스 통합 테스트 추가

### 설계 메모

- 상세 조회 엔드포인트는 Security 레벨에서는 열어두고, Service 계층에서 `visibility`와 현재 사용자 ID를 기준으로 접근 가능 여부를 판단합니다.
- 비공개 풀이 기록을 비로그인 또는 다른 사용자가 조회하면 `ACCESS_DENIED`를 반환합니다.
- 반례 조회 권한은 `#12` CounterExample API 구현 때 SolutionRecord 조회 권한과 같은 기준으로 연결합니다.
...
## #12 CounterExample API

### 브랜치

- `feature/#12-counter-example-api`

### 목적

풀이 과정에서 실패했던 입력과 오답 원인을 별도 반례로 기록할 수 있게 합니다.

### 작업 내용

- `CounterExampleRepository` 추가
- `CounterExampleCreateRequest`, `CounterExampleResponse` DTO 추가
- 반례 작성 API 추가
- 특정 풀이 기록의 반례 목록 조회 API 추가
- 반례 작성자 권한 검증 추가
- 공개/비공개 풀이 기록의 반례 조회 권한 적용
- 풀이 기록 삭제 시 연결된 반례 삭제 처리
- CounterExample API 통합 테스트 추가
